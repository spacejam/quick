...
func Check(f interface{}, config *Config) error {
	...
	shrinkQueue := [][]reflect.Value{}
	var lastFailing []reflect.Value

	j := 0
	for i := 0; i < maxCount; i++ {
		j = i
		if len(shrinkQueue) != 0 {
			// fmt.Println("shrinkQueue:", shrinkQueue)
			for i := range shrinkQueue {
				if len(shrinkQueue[i]) == 0 {
					continue
				}
				arguments[i], shrinkQueue[i] = shrinkQueue[i][0], shrinkQueue[i][1:]
				// arguments, shrinkQueue = shrinkQueue[0], shrinkQueue[1:]
			}
		} else {
		...
		// fmt.Println("calling with args", arguments)
		if !fVal.Call(arguments)[0].Bool() {
			lastFailing = arguments
			if newShrinkQueue, ok := tryShrink(arguments); ok {
				shrinkQueue = newShrinkQueue
				// fmt.Println("got shrunken:", shrinkQueue)
			} else {
	...
	if lastFailing != nil {
		return &CheckError{j + 1, toInterfaces(lastFailing)}
	}
...
func tryShrink(args []reflect.Value) (shrunkenArgs [][]reflect.Value, successful bool) {
	for _, arg := range args {
		s, ok := arg.Interface().(Shrinker)
		if ok {
			shrunkenValues := s.Shrink()
			if len(shrunkenValues) == 0 {
				ok = false
				shrunkenArgs = append(shrunkenArgs, []reflect.Value{arg})
			} else {
				shrunkenArgs = append(shrunkenArgs, shrunkenValues)
			}
		} else {
			shrunkenArgs = append(shrunkenArgs, []reflect.Value{arg})
		}
